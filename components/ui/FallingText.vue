...
interface FallingTextProps {
  ...
  fontSize?: string;
  /** CSS height for the container/canvas area, e.g. '40vh', '300px'. */
...
const setupTrigger = () => {
  ...
  if (props.trigger === 'scroll' && containerRef.value) {
    intersectionObserver = new IntersectionObserver(
      ...
      {
        // Trigger as soon as it enters, with a bottom margin so it can start earlier
        root: null,
...
const startPhysics = async () => {
  ...
  // Fallback if height is 0 (e.g., due to layout timing)
  if (height <= 0 && textRef.value) {
    const tRect = textRef.value.getBoundingClientRect();
    if (tRect.height > 0) {
      height = tRect.height;
      containerRef.value.style.minHeight = `${Math.ceil(height)}px`;
    }
  }
...
<template>
  <div ref="containerRef" class="relative w-full cursor-pointer text-center pt-8 overflow-hidden"
    :style="{ minHeight: props.containerHeight || undefined }"
